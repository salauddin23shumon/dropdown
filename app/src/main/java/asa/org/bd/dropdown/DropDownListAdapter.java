...
public class DropDownListAdapter extends BaseAdapter {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if(MainActivity.checkSelected[position])
	...
	private void setText(int position1){
		if (!MainActivity.checkSelected[position1]) {
			MainActivity.checkSelected[position1] = true;
			selectedCount++;
		} else {
			MainActivity.checkSelected[position1] = false;
			selectedCount--;
		}

		if (selectedCount == 0) {
			mSelectedItems.setText("selected");
		} else if (selectedCount == 1) {
			for (int i = 0; i < MainActivity.checkSelected.length; i++) {
				if (MainActivity.checkSelected[i] == true) {
